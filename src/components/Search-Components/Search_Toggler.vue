<template>
  <div class="w-full sm:w-auto">
    <div class="flex items-center justify-between sm:justify-evenly w-full sm:w-auto bg-white border-[0.1rem] 
              border-gray-300 p-2 rounded-full gap-3 shadow-md hover:shadow-lg cursor-pointer" 
      v-if="!showMainComponent"
      @click="emitShowMain()">

      <!--Default izgled togglera-->
      <div class="items-center gap-4 hidden sm:flex pl-3">
        <span href="" class="font-semibold">Anywhere</span>
          <p class="w-[0.12rem] h-6 bg-gray-300"></p>
        <span href="" class="font-semibold">Any week</span>
          <p class="w-[0.12rem] h-6 bg-gray-300"></p>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="p-2 sm:bg-orange-700 rounded-full">
...
<script>
import Search_Controler from './Search_Controller.vue';
export default {
  components:{
    Search_Controler
  },
...
.fade_rev-enter-active, .fad_rev-leave-active {
  transition: all .3s;
}
.fad_rev-enter, .fad_rev-leave-to{
  transform: translateY(-3rem);
  opacity: 0;
}
</style>