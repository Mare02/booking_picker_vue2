<template>
  <div class="flex flex-col items-center sm:max-h-fit max-sm:w-full">
    <div class="flex items-center gap-1 p-1 bg-gray-300 rounded-full w-80 font-medium mb-4">
      <span :class="{'bg-white hover:bg-white': selected_type == 1}" class="rounded-full text-center transition-all px-6 py-[0.3rem] cursor-pointer w-1/2 hover:bg-gray-200/90" @click="selectType" id="1">Choose dates</span>
      <span :class="{'bg-white hover:bg-white': selected_type == 2}" class="rounded-full text-center transition-all px-6 py-[0.3rem] cursor-pointer w-1/2 hover:bg-gray-200/90" @click="selectType" id="2">I'm flexible</span>
    </div>
    ...
    <div class="flex flex-col items-center max-sm:w-full" v-if="selected_type == 2">
      <div class="w-full flex flex-col max-sm:items-start items-center max-sm:px-6 max-sm:py-2 max-sm:border-y-[0.12rem]
                 border-gray-300">
        <span class="font-semibold sm:text-xl mb-2 sm:mb-4 sm:mt-2">Stay for a {{selected_flexible_opt}}</span>
        <div class="flex items-center gap-2">
          <div v-for="opt in flexible_options" :key="opt.id" class="px-2 py-0.5 sm:px-3 sm:py-1 border-[0.12rem] border-gray-300 hover:border-gray-400 rounded-full cursor-pointer"
                @click="selected_flexible_opt = opt.name" :class="{'border-[0.2rem] border-gray-500': selected_flexible_opt === opt.name}">
            {{opt.name}}
          </div>
        </div>
      </div>
      <div class="my-2 sm:mb-4 sm:mt-9 flex sm:justify-center w-full max-sm:px-6">
        <span class="font-semibold text-lg sm:text-xl">Go anytime</span>
      </div>
      
      <div class="flex items-center gap-4 pb-8 w-full sm:max-w-2xl max-sm:px-4 overflow-x-scroll">
        <div v-for="mon in months" :key="mon.monthId" class="flex flex-col justify-center items-center min-w-[7.5rem] sm:min-w-[8rem] min-h-[7rem] sm:min-h-[9rem] 
                                                            rounded-2xl border-[0.15rem] cursor-pointer"
          ...
          <span class="mt-2">{{mon.monthName}}</span>
          <span>{{mon.year}}</span>
        </div>
      </div>
    </div>
